#!/usr/bin/env python3
"""
Script para verificar la conexión a la instancia RDS MySQL
"""
...
def test_rds_connection(host, username, password, database, port=3306, max_retries=5, retry_interval=30):
    """
...
def main():
    parser = argparse.ArgumentParser(description='Prueba de conexión a AWS RDS MySQL')
    ...
    if not args.host:
        print("=" * 50)
        print("Prueba de Conexión a AWS RDS MySQL")
        print("=" * 50)
        ...
        port_input = input("Puerto (default: 3306): ").strip()
        if port_input:
            try:
